...
def launch_mob_cam():
    ...
    pdf_img = PIL.Image.open('./Icon/pdf.png')
...
def crop_image():
    ...
    cn = './Cropped_Images/img_' + str(crop_c) + '.jpg'
    cv2.imwrite(cn, img1)
    ...
    windo.after(15000, destroy_widget, display2)
    windo.after(15000, destroy_widget, imageFrame2)
    windo.after(15000, destroy_widget, imlab2)

    imgr = cv2.imread(cn)
    imgr = cv2.cvtColor(imgr, cv2.COLOR_BGR2GRAY)
    t = threshold_local(imgr, 17, offset=15, method='gaussian')
    imgr = (imgr > t).astype('uint8') * 255
    repn = Path('Processed_Images')
    if repn.is_dir():
        pass
    else:
        os.mkdir('Processed_Images')
    cn1 = './Processed_Images/img_' + str(crop_c) + '.jpg'
    ...
    windo.after(15000, destroy_widget, display4)
    windo.after(15000, destroy_widget, imageFrame4)
    windo.after(15000, destroy_widget, imlab4)
...
def pdf_gen():
    ...
    if launch == False:
        ...
        ## open the folder to save the pdf 
        repn = Path('Generated_PDF')
        if repn.is_dir():
            pass
        else:
            os.mkdir('Generated_PDF')#craete folder to save the saved pdf file

        ## Create First Intro page
        img = PIL.Image.new('RGB', (100, 30), color=(255, 255, 255))
        fnt = ImageFont.truetype('./Icon/arial.ttf', 13)
        d = ImageDraw.Draw(img)
        d.text((5, 10), "Scanned PDF ", font=fnt,fill=(0, 0, 0))
        img.save('./Processed_Images/z.jpg')
        scanned_imgs.append('./Processed_Images/z.jpg')
        ## Generate PDF-
        image_list = []
        ...
        img.save('./Generated_PDF/Scanned_'+str(pdf_c)+'_'+str(Hour)+'_'+str(Minute)+'_'+str(Second)+'.pdf',save_all=True, append_images=image_list)
        # img.save('./Scanned_PDF/Scanned_'+str(pdf_c)+'.pdf',save_all=True, append_images=image_list)
        noti = tk.Label(windo, text='PDF Generated Successfully!!', width=20, height=1, fg="black", bg="spring green",
                        font=('times', 13, ' bold '))
        noti.place(x=24, y=68)
        windo.after(2000, destroy_widget, noti)
        destroy_cam()

lab = tk.Label(windo, text="Enter your IP4 URL of IP webcam app", width=28, height=1, fg="white", bg="blue2",
               font=('times', 16, ' bold '))
lab.place(x=24, y=5)

txt = tk.Entry(windo, borderwidth=4, width=34, bg="white", fg="black", font=('times', 16, ' bold '))
txt.place(x=24, y=35)
txt.insert(0,'http://192.168.0.102:8080/shot.jpg')
